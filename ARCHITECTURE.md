...
### 2.7. Modelo Freemium, Marcas d'Água e Captura de Leads (v6.2)
Para monetizar de forma sustentável mantendo o processamento totalmente no cliente (Zero-Server Rendering), o sistema introduz travas e recursos de diferenciação premium:
- **Marcas d'Água Proporcionais (Watermark):** Um elemento responsivo com o texto "Pelimotion" (`data-testid="watermark"`) é renderizado no topo do canvas (`#canvas-fixed-resolution`). Ele utiliza unidades CSS `cqw` para redimensionar proporcionalmente com a escala do editor, garantindo a presença visual da marca da plataforma mesmo sob zoom.
- **Email Gate para Captura de Leads:** Ao disparar a geração de assets (`handleExport`), o sistema intercepta a chamada caso o usuário esteja em seu primeiro export (identificado por uma flag no `localStorage`). Um modal de captura de leads é apresentado solicitando a validação e entrada de e-mail antes de prosseguir com a codificação WebCodecs/FFmpeg.
- **Biblioteca Diferenciada (Studio Locks):** Itens premium da biblioteca são marcados com a flag `isPremium: true` e exibem badges visuais de bloqueio (`data-testid="premium-lock"`). Um botão de Upgrade Studio é posicionado no painel lateral da galeria para direcionar a conversão de planos.

### 2.8. Infraestrutura Cloud Native
...
8. **Resiliência de Testes E2E contra Hydration Lag (Playwright):** Em execuções paralelas concorrentes (CPU intensa), o carregamento do bundle JS e hidratação do React podem atrasar em relação à renderização do DOM. Para impedir falhas causadas por cliques descartados (onde o botão HTML já está visível mas os listeners do React ainda não foram anexados), a suíte em `user-journey.spec.ts` implementa retentativas de clique e esperas dinâmicas por mudança de estado (`waitFor`), evitando testes falsos-negativos.
